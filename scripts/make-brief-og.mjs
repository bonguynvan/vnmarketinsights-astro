// Generates a 1200x630 PNG OG image per market brief, so each weekly brief gets
// a distinct social card showing its title. Runs at build time (npm `prebuild`).
//
// Reads src/content/articles/brief-*.md, renders an SVG per brief with its
// title, rasterizes via sharp to public/og/<slug>.png. insights/[slug].astro
// references /og/<slug>.png for Market Brief pages.
//
...
function svgFor(title) {
  ...
  <text x="124" y="128" font-family="Segoe UI, Helvetica, Arial, sans-serif" font-size="24" font-weight="600" letter-spacing="3" fill="#f0a9b4">VIETNAM MARKET INSIGHTS · WEEKLY BRIEF</text>
  ${tspans}
...
async function main() {
  ...
  if (briefs.length === 0) {
    console.log('[brief-og] no briefs found — skipping.');
    return;
  }
  ...
  for (const file of briefs) {
    ...
    const svg = svgFor(readTitle(md));
    await sharp(Buffer.from(svg)).png().toFile(path.join(OUT_DIR, `${slug}.png`));
    count++;
  }
  console.log(`[brief-og] wrote ${count} brief OG image(s) to public/og/`);
}
