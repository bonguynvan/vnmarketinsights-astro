...
// ---------------------------------------------------------------------------
// Technical-only outlook (OHLC in, no fundamentals) — for real-data pages
// ---------------------------------------------------------------------------

export interface TechnicalOutlook {
  /** Technical sub-score, 0–50 (same rules as the composite's technical half). */
  score: number;
  /** Bullet-point reasons generated by the engine. */
  reasons: string[];
  /** Surfaced indicator values for display. 0 means "not enough data". */
  lastClose: number;
  ma50: number;
  ma200: number;
  rsi: number;
  week52High: number;
  week52Low: number;
  /** Where last close sits in the 52-week range, 0–100. */
  rangePositionPct: number;
  /** A coarse trend label derived from the score. */
  trend: 'Bullish' | 'Constructive' | 'Mixed' | 'Bearish';
  trendColor: 'green' | 'yellow' | 'orange' | 'red';
}

function trendFromScore(score: number): { trend: TechnicalOutlook['trend']; trendColor: TechnicalOutlook['trendColor'] } {
  if (score >= 40) return { trend: 'Bullish', trendColor: 'green' };
  if (score >= 28) return { trend: 'Constructive', trendColor: 'yellow' };
  if (score >= 16) return { trend: 'Mixed', trendColor: 'orange' };
  return { trend: 'Bearish', trendColor: 'red' };
}

/**
 * Compute a technical-only outlook (0–50) from OHLC bars alone. Mirrors the
 * technical half of the composite engine but surfaces the underlying indicator
 * values so real-data pages can show the numbers behind the read.
 */
export function technicalOutlook(bars: OHLCBar[]): TechnicalOutlook {
  const reasons: string[] = [];
  const score = scoreTechnical(bars, reasons);

  const prices = closePrices(bars);
  const lastIdx = prices.length - 1;
  const lastClose = lastIdx >= 0 ? prices[lastIdx] : 0;
  const ma50arr = calculateMA(prices, 50);
  const ma200arr = calculateMA(prices, 200);
  const rsiArr = calculateRSI(prices, 14);
  const { high: week52High, low: week52Low } = week52Range(bars);
  const rangePositionPct =
    week52High > week52Low ? ((lastClose - week52Low) / (week52High - week52Low)) * 100 : 0;

  const { trend, trendColor } = trendFromScore(score);
  return {
    score,
    reasons,
    lastClose,
    ma50: lastIdx >= 0 ? ma50arr[lastIdx] : 0,
    ma200: lastIdx >= 0 ? ma200arr[lastIdx] : 0,
    rsi: lastIdx >= 0 ? rsiArr[lastIdx] : 0,
    week52High,
    week52Low,
    rangePositionPct,
    trend,
